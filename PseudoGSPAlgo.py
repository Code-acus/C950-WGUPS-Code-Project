...
################################################################################################################################
    ...
    FUNCTION get_package_delivery_address()
        get_package_delivery_address = distance_name_csv

    # Calculate the total package deliver distance from the row/column values which is in O(1) time
    FUNCTION get_package_delivery_distance(row, col, total):
        DISTANCE = distance_csv(row) AND distance_csv(column)
        IF DISTANCE = RETURNED VALUE
            DISTANCE IS distance_csv(row) AND distance_csv(column)
