...
with open('./data/time_data.csv') as csvfile2:
    ...
    # Explain what you are doing with this code and why you are using the following code:
    def get_current_distance(row, column, total):
    ...
    def first_truck_indices[]
    # #
    def second_truck[]
    def second_truck_indices[]
    # #
    def third_truck[]
    def third_truck_indices[]
    ...
    def get_shortest_route(_list, num, curr_location):
        ...
        for i in _list:
            if get_current_distance(curr_location, value) <= lowest_value:
                lowest_value = get_current_distance(curr_location, value)
                location = value
